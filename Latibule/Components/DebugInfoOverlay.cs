...
public class DebugInfoOverlay : BaseComponent
{
    ...
    public override void OnRenderFrame(FrameEventArgs args)
    {
        ...
        if (EngineStates.EnabledDebugOverlays[DebugOverlayType.Info]) DrawDebugScreen(args);
    }
    ...
    {
        var gameWindow = EngineStates.GameWindow;
        string[] debugTexts =
        [
            $"",
    ...
    private void DrawTextArray(string[] texts, Vector2 startPosition, FSColor color, bool rightSide = false)
    {
        ...
        for (var i = 0; i < texts.Length; i++)
        {
            ...
            if (rightSide) startPosition.X = (EngineStates.GameWindow.ClientSize.X - font.MeasureString(text).X) - 5;
