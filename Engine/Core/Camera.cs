...
public static float Fov = EngineDefaults.FIELD_OF_VIEW_DEGREES; // Field of view in degrees
    ...
    public Camera(Vector3 position, Vector3 target, Vector3 eyePosition, float aspectRatio)
    {
    ...
    {
        if (EngineStates.MouseLookLocked) return;

        var deltaX = EngineStates.MState.Delta.X;
        var deltaY = EngineStates.MState.Delta.Y;
        ...
        _yaw -= deltaX * sensitivity; // Invert Y-axis for a natural feel
