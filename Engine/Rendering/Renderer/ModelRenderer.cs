...
public class ModelRenderer : IRenderable, IDisposable
{
    private readonly Shader _shader;
    private readonly Scene _scene;
    private readonly Transform _transform;
    protected Texture _fallback;

    protected IReadOnlyList<Texture>? _texturesByMaterial;
    protected Texture? _forceAllTexture;

    private readonly List<GpuMesh> _meshes = [];

    protected readonly List<(GpuMesh mesh, Matrix4 model)> _opaque = new();
    protected readonly List<(GpuMesh mesh, Matrix4 model, float sortKey)> _transparent = new();
    ...
    protected void Draw(GpuMesh mesh, Matrix4 model)
    {
    ...
    protected void BuildGpuMeshes(Scene scene)
    {
        _meshes.Clear();
    ...
    protected sealed class GpuMesh(
